...
**원인:** 두 브랜치가 공통 조상이 없을 때 발생

#### 발생 상황

##### 1. GitHub에서 ZIP 다운로드 후 작업
```bash
# GitHub에서 Download ZIP 클릭
# 압축 해제 후
git init                      # .git 폴더 없음 → 새 저장소 생성
git commit -m "first"
git push
# ! [rejected] unrelated histories
```

**문제:** ZIP 파일에는 `.git` 폴더가 없어서 Git 이력이 끊김

##### 2. GitHub에서 README 생성 + 로컬 init
```bash
# GitHub에서 "Initialize with README" 체크 → 커밋 생성
# 로컬에서
git init
git commit -m "first"
git remote add origin URL
...
##### 3. 완전히 다른 저장소 병합 시도
```bash
git checkout --orphan new-branch  # 부모 없는 브랜치
git commit -m "new"
git merge main
...
##### 1. **절대 ZIP 다운로드 쓰지 말 것**
```bash
# 잘못된 방법
Download ZIP 클릭 → 압축 해제 → git init

# 올바른 방법
git clone https://github.com/유저명/저장소명.git
```

##### 2. GitHub 저장소 생성 시 빈 상태로 만들기
- README 체크 안 함
- .gitignore 선택 안 함
- License 선택 안 함
...
##### 3. 기존 저장소 받을 때는 무조건 Clone
```bash
git clone https://github.com/유저명/저장소명.git
```

#### Clone vs Download ZIP 차이

| 방법 | .git 폴더 | Git 이력 | push/pull | 병합 |
|------|-----------|----------|-----------|------|
| **git clone** | O | O | O | O |
| **Download ZIP** | X | X | 새 저장소 필요 | unrelated histories 에러 |

**핵심: 무조건 `git clone` 사용할 것!**
